...
public partial struct InteractionCertificationAlgorithms
{
    ...
    {
        JobHandle handle = CreatePreparePersistentClassificationJob(runtimeState).Schedule(dependency);
        ...
        return CreateCommitPersistentClassificationJob(runtimeState).Schedule(handle);
    }
        ...
        NativeReference<ContactPipelineExecutionState> runtimeState)
    {
        ...
        Diagnostics.PersistentClassificationTelemetry.Value = telemetry;
#endif
        ...
        {
            Persistent.PersistentClassificationState.Value = phase;
            return;
        }
        ...
        ref IncrementalContactPipelineStatistics incremental)
    {
             ...
             pairIndex++)
        {
            ...
            if (PersistentContactIndex.IsCreated)
                Persistent.PersistentContactIndex[result.Contact.Key] = result.Contact;
            if (result.WasReclassified != 0)
            {
                incremental.ReclassifiedPairEvaluationCount++;
                incremental.SweptClassificationEvaluationCount++;
    ...
    private bool RebuildPersistentSpatialMembership(uint targetEpoch)
    {
        ...
        if (requiredEntryCount > PersistentSpatialMembership.Capacity)
        {
            ...
            Persistent.PersistentSpatialMembershipEpoch.Value = uint.MaxValue;
            return false;
        }
        ...
        Persistent.PersistentSpatialMembershipEpoch.Value = targetEpoch;
        return true;
    }
        ...
        ref IncrementalContactPipelineStatistics incrementalStatistics)
    {
        ...
        if (stamp == 0u)
        {
            for (int i = 0; i < PersistentSpatialVisitStampByProxy.Length; i++)
                Persistent.PersistentSpatialVisitStampByProxy[i] = 0u;
            stamp = 1u;
        }
        Persistent.PersistentSpatialVisitStamp.Value = stamp;
        if ((uint)dirtyProxyIndex < (uint)PersistentSpatialVisitStampByProxy.Length)
            Persistent.PersistentSpatialVisitStampByProxy[dirtyProxyIndex] = stamp;
            ...
            for (int y = minCell.y; y <= maxCell.y; y++)
            {
                ...
                do
                {
                    if ((uint)otherProxyIndex >=
                        ...
                        continue;
                    Persistent.PersistentSpatialVisitStampByProxy[otherProxyIndex] = stamp;
