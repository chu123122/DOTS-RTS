...
public partial struct InteractionCertificationAlgorithms
{
        ...
        NativeReference<ContactPipelineExecutionState> runtimeState)
    {
             ...
             pairIndex++)
        {
            ...
            // 新分类结果写入 O(1) 索引
            if (PersistentContactIndex.IsCreated)
                Persistent.PersistentContactIndex[contact.Key] = contact;
    ...
    private void RebuildPersistentIncidentPairLookupIfNeededParallel()
    {
        ...
        if (requiredEntryCount > PersistentIncidentPairLookup.Capacity)
        {
            ...
            Persistent.PersistentIncidentLookupEpoch.Value = uint.MaxValue;
            return;
        }
        ...
        Persistent.PersistentIncidentLookupEpoch.Value = epoch;
    }
    ...
    private void InvalidateSoftIncidentIndex()
    {
        ...
        state.SoftIsValid = 0;
        Solver.ActiveIncidentIndexState.Value = state;
    }
