...
public partial struct InteractionCertificationAlgorithms
{
        ...
        out bool persistentViewReady)
    {
        ...
        if (!mapped)
        {
            ...
            Persistent.IncrementalCacheState.Value = invalidState;
    ...
    private bool TryBuildCurrentContactViewsFromPersistentState()
    {
        ...
        Persistent.PredictiveContactScheduleCursor.Value = 0;
        if (Pairs.Length > 1)
    ...
    private void RebuildPersistentContactViews()
    {
        ...
        Persistent.IncrementalCacheState.Value = state;
    }
        ...
        int scheduleStartSubstep)
    {
        ...
        IncrementalContactCacheState cacheState = IncrementalCacheState.Value;
        cacheState.ClassificationEpoch = classificationEpoch;
        Persistent.IncrementalCacheState.Value = cacheState;
    }
        ...
        ref IncrementalContactPipelineStatistics incrementalStatistics)
    {
        ...
        Persistent.PredictiveContactScheduleCursor.Value = cursor;
        if (addedPair)
        ...
        ushort nextCheckSubstep)
    {
        ...
        Persistent.PersistentPredictiveContacts[contactIndex] = contact;
    ...
    private void RebuildPersistentContactIndex()
    {
        ...
        for (int i = 0; i < PersistentPredictiveContacts.Length; i++)
        {
            ...
            Persistent.PersistentContactIndex[c.Key] = c;
        }
    }
        ...
        out int topologyDirtyCount)
    {
        ...
        for (int dirtyIndex = 0; dirtyIndex < IncrementalDirtyBodies.Length; dirtyIndex++)
        {
            ...
            if ((merged & IncrementalBodyDirtyFlags.EntitySet) != 0)
                return false;
            dirty.Flags = merged;
            Persistent.IncrementalDirtyBodies[dirtyIndex] = dirty;
            Persistent.IncrementalDirtyFlagsByBody[bodyIndex] = (byte)merged;
        }
        SummarizePreparedIncrementalDirtyBodies(
    ...
    private void RebuildPersistentProxyIndexByBody()
    {
        ...
        for (int bodyIndex = 0; bodyIndex < Bodies.Length; bodyIndex++)
            Persistent.PersistentProxyIndexByBody[bodyIndex] = -1;
        for (int proxyIndex = 0; proxyIndex < PersistentSweptProxies.Length; proxyIndex++)
        {
            ...
            if ((uint)bodyIndex < (uint)PersistentProxyIndexByBody.Length)
                Persistent.PersistentProxyIndexByBody[bodyIndex] = proxyIndex;
        }
    }
        ...
        ref IncrementalContactPipelineStatistics incrementalStatistics)
    {
        ...
        for (int dirtyIndex = 0; dirtyIndex < IncrementalDirtyBodies.Length; dirtyIndex++)
        {
            ...
            Body.Bodies[bodyIndex] = stateSnapshot;
            Body.NavigationStates[bodyIndex] = stateNavigation;
            Body.MotionIntents[bodyIndex] = stateIntent;
            Body.MotionEvidence[bodyIndex] = stateEvidence;
            Body.StepStates[bodyIndex] = stateStep;
        }
        IncrementalContactCacheState cacheState = IncrementalCacheState.Value;
        ...
        int scheduleStartSubstep)
    {
        ...
        for (int pairIndex = 0; pairIndex < rawPairCount; pairIndex++)
        {
            ...
            Views.Pairs[activeWriteIndex++] = BuildContactConstraintFromPersistentContact(
                rawPair.BodyA,
                rawPair.BodyB,
                contact);
        }
    ...
    private void RemoveDirtyPredictiveContactSchedules()
    {
             ...
             scheduleIndex++)
        {
            ...
            if (IsDirtyEntity(entry.Key.EntityA) ||
                ...
                continue;
            Persistent.PredictiveContactSchedule[writeIndex++] = entry;
        }
