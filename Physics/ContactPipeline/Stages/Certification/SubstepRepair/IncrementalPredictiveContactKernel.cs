...
internal partial struct CertificationStageKernel
{
        ...
        int scheduleStartSubstep)
    {
        ...
        float candidateDistance = radiusSum + math.max(0f, Configuration.PredictiveSkin);
        ...
        if (minDistanceSq > retainedDistance * retainedDistance ||
            (startDistanceSq > radiusSumSq && !Configuration.EnablePredictivePairGeneration))
        {
        ...
        else
        {
            ...
            lifecycle = preventSideExchange && Configuration.EnablePredictiveContacts
        ...
        {
            int totalSubstepCount = math.max(1, Configuration.SubstepCount);
        ...
        out ContactConstraint pair)
    {
        ...
        pair = new ContactConstraint
        {
            BodyA = bodyAIndex,
            BodyB = bodyBIndex,
            ContactMode = preventSideExchange && Configuration.EnablePredictiveContacts
                ? ContactConstraintMode.Predictive
                : ContactConstraintMode.Regular,
    ...
    private bool IsPersistentCacheStructurallyReusable() =>
        PersistentCacheReusability.IsStructurallyReusable(
            ...
            new PersistentCacheReusability.ConfigurationFingerprint
            {
                ...
                PredictivePairGenerationEnabled = Configuration.EnablePredictivePairGeneration,
                PredictiveContactsEnabled = Configuration.EnablePredictiveContacts,
    ...
    private void PrepareInitialPersistentDirtyBodySet()
    {
        ...
        for (int bodyIndex = 0; bodyIndex < Bodies.Length; bodyIndex++)
        {
            IncrementalBodyDirtyFlags flags = ClassifyAndUpdatePersistentProxyForBody(
                ...
                Configuration.GuardEnvelopeMargin, Configuration.SoftAvoidanceShell, Configuration.SoftAvoidanceResponseRate,
                Configuration.SoftAvoidanceVelocitySolver, Configuration.RvoTimeHorizon);
            if (flags != IncrementalBodyDirtyFlags.None)
                SetIncrementalDirtyFlags(bodyIndex, flags);
        }
    }

    private bool ConsumeDirtyBodyRefreshSummary(
        ref IncrementalContactPipelineStatistics incrementalStatistics,
        out int topologyDirtyCount)
    {
        DirtyBodyRefreshSummary summary = DirtyBodyRefreshSummary.Value;
        topologyDirtyCount = summary.TopologyDirtyCount;
        ...
        incrementalStatistics.MotionDirtyBodyCount += summary.MotionDirtyCount;
        return summary.IsReusable != 0 && summary.EntitySetDirty == 0;
    }
    ...
    private void ClassifyAndPatchDirtyIncidentContacts(
        ...
        int scheduleStartSubstep)
    {
        // PredictiveContactScratch、PredictiveContactSchedule 与索引已由
        // DirtyContactSchedule Count/Prefix/Scatter/Commit 阶段压紧。
    ...
    private void BuildCurrentIncrementalSweptProxies()
    {
        ...
        float guardMargin = math.max(0f, Configuration.GuardEnvelopeMargin);

        for (int bodyIndex = 0; bodyIndex < Bodies.Length; bodyIndex++)
        {
            CurrentIncrementalProxies.Add(BuildPersistentProxyFromState(
                bodyIndex, Bodies[bodyIndex], MotionEvidence[bodyIndex],
                StepStates[bodyIndex], guardMargin, Configuration.SoftAvoidanceShell,
                Configuration.SoftAvoidanceResponseRate, Configuration.SoftAvoidanceVelocitySolver,
                Configuration.RvoTimeHorizon));
        }
    ...
    private void CalculateIncrementalTightSweptBounds(
        ...
        PersistentContactMath.CalculateIncrementalTightSweptBounds(
            ...
            Configuration.PredictiveSkin, Configuration.TimestepContactMargin, Configuration.SoftAvoidanceShell,
            Configuration.RvoTimeHorizon, Configuration.SoftAvoidanceVelocitySolver,
        ...
        CrowdBodyStepState stateStep)
    {
        if (Configuration.SoftAvoidanceVelocitySolver !=
            ...
            Configuration.SoftAvoidanceShell <= 0f || Configuration.SoftAvoidanceResponseRate <= 0f)
            return stateEvidence.BaselineEnd.xz;
        return stateEvidence.TrajectoryStart.xz +
               stateStep.BaseVelocity.xz * math.max(0f, Configuration.RvoTimeHorizon);
    }
        ...
        ref IncrementalContactPipelineStatistics incrementalStatistics)
    {
        ...
        state.BodyCount = Bodies.Length;
        state.NeighborPairCount = PersistentNeighborPairs.Length;
        state.GuardMargin = math.max(0f, Configuration.GuardEnvelopeMargin);
        state.PredictiveSkin = math.max(0f, Configuration.PredictiveSkin);
        state.TimestepContactMargin = math.max(0f, Configuration.TimestepContactMargin);
        state.SoftAvoidanceShell = math.max(0f, Configuration.SoftAvoidanceShell);
        state.SoftAvoidanceResponseRate = math.max(0f, Configuration.SoftAvoidanceResponseRate);
        state.RvoTimeHorizon = math.max(0f, Configuration.RvoTimeHorizon);
        state.SubstepCount = math.max(1, Configuration.SubstepCount);
