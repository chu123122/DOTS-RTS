...
- Runtime observation data contracts used by scheduled jobs live in
  `Physics/ContactPipeline/Observability/Contracts`; they are data-only and never
  control simulation.
- Solver-side capture fragments live with their owner at
  `Physics/ContactPipeline/Stages/Solver/Observability` and are compile-gated.
...
- `Recording/` and runtime experiments consume immutable publication.
- UI-facing panels, overlays and experiment controls live under
  `UI/Diagnostics`; they only consume immutable Physics publications.
