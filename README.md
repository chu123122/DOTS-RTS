...
> **实测结果：** 在 1,000 单位开阔往返的高密度持续接触场景中（4 Substeps × 4 Iterations，跨帧缓存关闭），每种配置独立采样 2 次、每次 1,200 帧。仅启用 Substep 接触集缓存后：
>
...
>
> 性能提升并非通过减少接触约束换取：平均接触 Pair 从 **596** 增至 **721**，平均活跃 Pair 从 **543** 增至 **672**。在处理更多持续接触的同时，求解总耗时仍降低 **62.7%**，并改善了高密度拥挤时的碰撞连续性与稳定性。
